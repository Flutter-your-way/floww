...
class FlowScoreBoostCard extends StatelessWidget {
  const FlowScoreBoostCard({super.key, required this.boosts, this.onBoostTap});

  final List<FlowScoreBoost> boosts;
  final ValueChanged<FlowScoreBoost>? onBoostTap;
      ...
      child: Column(
        ...
        children: [
          ...
          for (var i = 0; i < boosts.length; i++) ...[
            ...
            _BoostRow(
              boost: boosts[i],
              onTap: onBoostTap == null ? null : () => onBoostTap!(boosts[i]),
            ),
          ],
        ],
      ),
...
class _BoostRow extends StatelessWidget {
  const _BoostRow({required this.boost, this.onTap});
